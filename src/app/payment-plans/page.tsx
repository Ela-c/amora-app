	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (result.error) {
			...
			if (
				...
				setPaymentMessage(
					"Payment successful! Your page should redirect soon or you will see a success message on the redirected page."
				);
...
export default function PaymentPlansPage() {
	...
	const [clientSecretForPayment, setClientSecretForPayment] = useState<
		string | null
	>(null);
		...
		const fetchUser = async () => {
			try {
				...
				if (response.status === 401) {
					console.log(
						"/api/auth/me returned 401, redirecting to login."
					);
				...
				if (!response.ok) {
					...
					if (
						...
						console.error(
							"Received non-JSON error response from /api/auth/me:",
							errorText
						);
					}
				...
				if (contentType && contentType.includes("application/json")) {
					...
					console.error(
						"Received non-JSON success response from /api/auth/me:",
						responseText
					);
	...
	const handleCompleteCheckout = async () => {
		if (selectedPlanName) {
			...
			if (selectedPlanName === "Free Spirit") {
				console.log(
					"Free Spirit plan selected, redirecting to dashboard."
				);
			...
			if (amountInCents === null || amountInCents <= 0) {
				...
				console.error(
					"Could not parse price for the selected plan or price is invalid"
				);
				setError(
					"There was an issue with the plan pricing. Please contact support."
				);
				return;
			}
			...
			try {
				...
				if (!clientSecret) {
					throw new Error(
						"Received an invalid client secret from the server."
					);
				}
	...
	const elementsOptions: StripeElementsOptions | undefined =
		clientSecretForPayment
			? {
					clientSecret: clientSecretForPayment,
				...
				{!showPaymentForm && (
					<>
						...
						<p className="text-xl text-center text-gray-600 mb-12">
							Find the perfect plan to connect and spark something
							new.
						</p>
								...
								<div
									...
									className={`bg-white rounded-xl shadow-lg p-8 flex flex-col ${
										selectedPlanName === plan.name
											? "border-2 border-blue-600 ring-4 ring-blue-600 ring-opacity-30"
